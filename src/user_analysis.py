...
class User_Analysis():
    ...
    def user_profile_description(self):
        ...
        genre_value_counts, cat_value_counts = self.plot_genre_cat_wordcloud()
    ...
    def plot_genre_cat_preferences(self, axes, x, y):
        cat_list =  self.games_info['categories'].tolist()
        genre_list = self.games_info['genres'].tolist()
        ...
        pd.Series(genres).value_counts(normalize=True, sort=True, ascending=False)[:10].plot(ax=axes[x,y], kind='bar', title ="Actual", fontsize = 5)
        #pd.Series(categories).value_counts(sort=True, ascending=False).plot(kind='bar')

    def plot_genre_cat_wordcloud(self):
        cat_list =  self.games_info['categories'].tolist()
        genre_list = self.games_info['genres'].tolist()
